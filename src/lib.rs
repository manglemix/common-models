...
impl<B: Backend, M1: Record<B>, M2: Record<B>> Record<B> for TwoTuple<M1, M2> {
    ...
    fn from_item<S: burn::record::PrecisionSettings>(
        ...
        Self(Record::from_item(m1, device), Record::from_item(m2, device))
    }
}
...
impl<B: Backend, M1: Record<B>, M2: Record<B>, M3: Record<B>> Record<B> for ThreeTuple<M1, M2, M3> {
    ...
    fn from_item<S: burn::record::PrecisionSettings>(
        (m1, m2, m3): Self::Item<S>,
        device: &B::Device,
    ) -> Self {
        Self(
            Record::from_item(m1, device),
            Record::from_item(m2, device),
            Record::from_item(m3, device),
        )
    }
}
...
impl<B: Backend> Record<B> for Activation {
    ...
    fn from_item<S: burn::record::PrecisionSettings>(item: Self::Item<S>, _: &B::Device) -> Self {
        item
    }
}
